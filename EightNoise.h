...
public:
	EightNoise(long int topLeftXCoord, long int topLeftYCoord, long int bottomRightXCoord, long int bottomRightYCoordlong,
			  unsigned long int pixelsPerChunk, long int seed, unsigned int noiseLimiter, unsigned short int octaveCount);
...
void EightNoise<value>::fillPixelMap() {
	...
	unsigned long int vertexCount = 1 + ((xSideLength - xPadding) % pixelsPerChunk) ? ((xSideLength / pixelsPerChunk) + 1) : (xSideLength /pixelsPerChunk) + (xPadding == 0) ? (0) : (1);
